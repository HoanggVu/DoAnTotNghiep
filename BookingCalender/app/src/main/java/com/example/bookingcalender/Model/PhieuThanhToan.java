package com.example.bookingcalender.Model;

import java.io.Serializable;

public class PhieuThanhToan implements Serializable {
    String accountIdUser;
    String accountIdDoctor;
    String avatarUser;
    String nameUser;
    String dienThoaiUser;
    String avatarDoctor;
    String khoaDoctor;
    String nameDoctor;
    String ngayLV;
    String tGLVTu;
    String tGLVDen;
    String trangThai;
    String dichVuKham;
    String kLTrieuChung;
    String chanDoan;
    String lieuPhap;
    String donThuoc;
    String tGKham;
    String ngayKham;
    String tGThanhToan;
    String ngayThanhToan;
    String thanhTien;

    public PhieuThanhToan(){}

    public PhieuThanhToan(String accountIdUser, String accountIdDoctor, String avatarUser, String nameUser,
                          String dienThoaiUser, String avatarDoctor, String khoaDoctor, String nameDoctor,
                          String ngayLV, String tGLVTu, String tGLVDen, String trangThai, String dichVuKham,
                          String kLTrieuChung, String chanDoan, String lieuPhap, String donThuoc, String tGKham,
                          String ngayKham, String tGThanhToan, String ngayThanhToan, String thanhTien) {
        this.accountIdUser = accountIdUser;
        this.accountIdDoctor = accountIdDoctor;
        this.avatarUser = avatarUser;
        this.nameUser = nameUser;
        this.dienThoaiUser = dienThoaiUser;
        this.avatarDoctor = avatarDoctor;
        this.khoaDoctor = khoaDoctor;
        this.nameDoctor = nameDoctor;
        this.ngayLV = ngayLV;
        this.tGLVTu = tGLVTu;
        this.tGLVDen = tGLVDen;
        this.trangThai = trangThai;
        this.dichVuKham = dichVuKham;
        this.kLTrieuChung = kLTrieuChung;
        this.chanDoan = chanDoan;
        this.lieuPhap = lieuPhap;
        this.donThuoc = donThuoc;
        this.tGKham = tGKham;
        this.ngayKham = ngayKham;
        this.tGThanhToan = tGThanhToan;
        this.ngayThanhToan = ngayThanhToan;
        this.thanhTien = thanhTien;
    }

    public String getAccountIdUser() {
        return accountIdUser;
    }

    public void setAccountIdUser(String accountIdUser) {
        this.accountIdUser = accountIdUser;
    }

    public String getAccountIdDoctor() {
        return accountIdDoctor;
    }

    public void setAccountIdDoctor(String accountIdDoctor) {
        this.accountIdDoctor = accountIdDoctor;
    }

    public String getAvatarUser() {
        return avatarUser;
    }

    public void setAvatarUser(String avatarUser) {
        this.avatarUser = avatarUser;
    }

    public String getNameUser() {
        return nameUser;
    }

    public void setNameUser(String nameUser) {
        this.nameUser = nameUser;
    }

    public String getDienThoaiUser() {
        return dienThoaiUser;
    }

    public void setDienThoaiUser(String dienThoaiUser) {
        this.dienThoaiUser = dienThoaiUser;
    }

    public String getAvatarDoctor() {
        return avatarDoctor;
    }

    public void setAvatarDoctor(String avatarDoctor) {
        this.avatarDoctor = avatarDoctor;
    }

    public String getKhoaDoctor() {
        return khoaDoctor;
    }

    public void setKhoaDoctor(String khoaDoctor) {
        this.khoaDoctor = khoaDoctor;
    }

    public String getNameDoctor() {
        return nameDoctor;
    }

    public void setNameDoctor(String nameDoctor) {
        this.nameDoctor = nameDoctor;
    }

    public String getNgayLV() {
        return ngayLV;
    }

    public void setNgayLV(String ngayLV) {
        this.ngayLV = ngayLV;
    }

    public String gettGLVTu() {
        return tGLVTu;
    }

    public void settGLVTu(String tGLVTu) {
        this.tGLVTu = tGLVTu;
    }

    public String gettGLVDen() {
        return tGLVDen;
    }

    public void settGLVDen(String tGLVDen) {
        this.tGLVDen = tGLVDen;
    }

    public String getTrangThai() {
        return trangThai;
    }

    public void setTrangThai(String trangThai) {
        this.trangThai = trangThai;
    }

    public String getDichVuKham() {
        return dichVuKham;
    }

    public void setDichVuKham(String dichVuKham) {
        this.dichVuKham = dichVuKham;
    }

    public String getkLTrieuChung() {
        return kLTrieuChung;
    }

    public void setkLTrieuChung(String kLTrieuChung) {
        this.kLTrieuChung = kLTrieuChung;
    }

    public String getChanDoan() {
        return chanDoan;
    }

    public void setChanDoan(String chanDoan) {
        this.chanDoan = chanDoan;
    }

    public String getLieuPhap() {
        return lieuPhap;
    }

    public void setLieuPhap(String lieuPhap) {
        this.lieuPhap = lieuPhap;
    }

    public String getDonThuoc() {
        return donThuoc;
    }

    public void setDonThuoc(String donThuoc) {
        this.donThuoc = donThuoc;
    }

    public String gettGKham() {
        return tGKham;
    }

    public void settGKham(String tGKham) {
        this.tGKham = tGKham;
    }

    public String getNgayKham() {
        return ngayKham;
    }

    public void setNgayKham(String ngayKham) {
        this.ngayKham = ngayKham;
    }

    public String gettGThanhToan() {
        return tGThanhToan;
    }

    public void settGThanhToan(String tGThanhToan) {
        this.tGThanhToan = tGThanhToan;
    }

    public String getNgayThanhToan() {
        return ngayThanhToan;
    }

    public void setNgayThanhToan(String ngayThanhToan) {
        this.ngayThanhToan = ngayThanhToan;
    }

    public String getThanhTien() {
        return thanhTien;
    }

    public void setThanhTien(String thanhTien) {
        this.thanhTien = thanhTien;
    }
}
